#!/usr/bin/env python3
"""
Recursively find all `seed_word.json` files and compile them into one `lang_word.json`
with the structure:
...
Notes:
- If a seed_word.json contains a single object, it is appended as one child.
- If a seed_word.json contains a list of objects, each object is appended.
...
def _as_children_items(data: JsonRoot, path: Path) -> List[JsonObj]:
    """
    Normalize seed_word.json content into a list of objects to append into `children`.
    Accepts either:
      - a single object
      - a list of objects
    """
...
def compile_to_lang_word(opts: Options) -> JsonObj:
    word_files = sorted(opts.root.rglob("seed_word.json"))
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Compile all seed_word.json files into a single lang_word.json"
    )
    ...
    ap.add_argument(
        "--strict",
        action="store_true",
        help="Stop immediately if any seed_word.json is invalid",
    )
    args = ap.parse_args()
    ...
    print(f"seed_word.json files found: {len(list(opts.root.rglob('seed_word.json')))}")
